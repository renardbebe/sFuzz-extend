...
bool str_is_equal(string s1, string s2) {  // dismiss case sensitive
  size_t pos = s1.rfind(":");
  if (pos != string::npos) {
    s1 = s1.substr(pos+1);
  }
...
ContractInfo parseJson(string jsonFile, string contractName, bool isMain) {
  ...
  //   if (str_is_equal(key.first, contractName)) {
  //     fullContractName = key.first;
  //     break;
  //   }
  // }
  fullContractName = root.get_child("contracts").back().first;   // use the last contract
